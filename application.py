...
def generate_personas_batch():
    try:
        ...
        logging.info(f"Received batch input: {data}")

        # Validate input format
        if not data or not isinstance(data.get("data"), list):
            return jsonify({"error": "Invalid input format. Expected a JSON object with a 'data' key containing a list of reviews."}), 400
        ...
        logging.info(f"Generated personas: {results}")
        return jsonify({"status": "success", "data": results}), 200
    except Exception as e:
        logging.error(f"Error processing batch input: {str(e)}")
        return jsonify({"error": str(e)}), 500
